# US-036: Bulk-Aktionen

## Datei: src/app/leads/actions.ts
...
## Datei: src/components/leads/columns.tsx (Checkbox-Spalte hinzufuegen)

Am Anfang des columns Arrays eine Checkbox-Spalte einfuegen:

```tsx
// Erste Spalte im ColumnDef Array:
{
  id: "select",
  header: ({ table }) => (
    <Checkbox
...
## Datei: src/components/leads/BulkActions.tsx
...
export function BulkActions({ selectedIds, onClear }: BulkActionsProps) {
  ...
  return (
    <>
      <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-zinc-900 border border-zinc-700 rounded-xl shadow-xl px-4 py-3 flex items-center gap-3">
