    ...
    <section className="bg-cream py-20 md:py-28 px-6 md:px-12 lg:px-16">
      <div className="max-w-6xl grid grid-cols-1 lg:grid-cols-[0.9fr_1.1fr] gap-12 lg:gap-20">
        <div>
          ...
          <h2 className="font-display font-light italic text-[clamp(2.2rem,4vw,4rem)] leading-tight text-ink mb-6">
            {title}
          </h2>
          <p className="text-mid text-sm md:text-base leading-relaxed max-w-xl">
            {summary}
          </p>
          <ul className="mt-8 space-y-3">
            {facts.map((fact) => (
              <li key={fact} className="flex gap-3 text-sm text-mid leading-relaxed">
        ...
        <div className="divide-y divide-stone/50 border-y border-stone/50">
          {questions.map((item) => (
            <article key={item.question} className="py-6">
              ...
              <p className="mt-3 text-sm md:text-base leading-relaxed text-mid">
                {item.answer}
              </p>
            </article>
          ))}
