...
export function LocalServicePage({ page }: { page: LocalSeoPage }) {
  return (
    <>
      ...
      <section className="bg-cream px-6 py-20 md:px-12 md:py-28 lg:px-16">
        <div className="grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          <div>
            ...
            <h2 className="font-display text-[clamp(2.2rem,4vw,4rem)] font-light italic leading-tight text-ink">
              Was diese Seite beantwortet.
            </h2>
          </div>
          <div className="space-y-4">
            {page.facts.map((fact) => (
              <p key={fact} className="border-b border-stone/50 pb-4 text-sm leading-relaxed text-mid md:text-base">
                {fact}
              </p>
            ))}
          </div>
      ...
      <section className="bg-parchment px-6 py-20 md:px-12 md:py-28 lg:px-16">
        <div className="max-w-6xl divide-y divide-stone/50">
          {page.sections.map((section) => (
            <article key={section.title} className="grid grid-cols-1 gap-6 py-10 md:grid-cols-[0.7fr_1.3fr] md:gap-16">
              <h2 className="text-xl font-medium tracking-tight text-ink">
                {section.title}
              </h2>
              <p className="max-w-2xl text-sm leading-relaxed text-mid md:text-base">
                {section.body}
              </p>
            </article>
          ))}
      ...
      <section className="bg-cream px-6 py-20 md:px-12 md:py-28 lg:px-16">
        <div className="grid max-w-6xl grid-cols-1 gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          ...
          <div className="divide-y divide-stone/50 border-y border-stone/50">
            {page.faq.map((item) => (
              <article key={item.question} className="py-6">
                <h3 className="text-base font-medium tracking-tight text-ink md:text-lg">
                  {item.question}
                </h3>
                <p className="mt-3 text-sm leading-relaxed text-mid md:text-base">
                  {item.answer}
                </p>
              </article>
            ))}
            <div className="flex flex-col gap-3 py-6 sm:flex-row">
              <Link
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-full bg-ink px-6 py-3 text-xs font-medium uppercase tracking-[0.14em] text-cream transition-opacity hover:opacity-80"
              >
                Anfrage starten
              </Link>
              <Link
                href={page.parentPath}
                className="inline-flex min-h-11 items-center justify-center rounded-full border border-stone/60 px-6 py-3 text-xs font-medium uppercase tracking-[0.14em] text-ink transition-colors hover:border-ink"
              >
                {page.parentLabel}
              </Link>
            </div>
